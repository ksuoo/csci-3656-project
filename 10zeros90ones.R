...
data_vector <- c(rep(0, 0.1 * n_elements), runif(0.9 * n_elements))  # Random numbers between 0 and 1
...
p_pca <- ggplot(df, aes(x = PC1, y = PC2, colour = label)) +
  ggtitle("PCA on sparse matrix (10 % zeros and 90 % rand(0, 1))") +
...
p_spca <- ggplot(df, aes(x = SPC1, y = SPC2, colour = label)) +
  geom_point(alpha = 0.5) +
  ggtitle("SPCA on Simulated Data for Varying L1 Penalty Terms (10 % zeros and 90 % rand(0, 1))") +
...
p_cpca <- ggplot(cpca_df, aes(x = cPC1, y = cPC2, colour = label)) +
  geom_point(alpha = 0.5) +
  ggtitle("cPCA of Simulated Data (10 % zeros and 90 % rand(0, 1))") +
  theme_minimal()
p_cpca
...
p_scpca <- ggplot(scpca_df, aes(x = scPC1, y = scPC2, colour = label)) +
  geom_point(alpha = 0.5) +
  ggtitle("scPCA of Simulated Data (10 % zeros and 90 % rand(0, 1))") +
  theme_minimal()
p_scpca
